...
class Login_model extends CI_Model
{
    ...
    {
        $this->db->select('tbl_members.userId, tbl_members.email, tbl_members.name, tbl_members.roleId, Roles.role');
        $this->db->from('tbl_members');
        $this->db->join('tbl_roles as Roles','Roles.roleId = tbl_members.roleId', 'left');        
        $this->db->where('tbl_members.isDeleted', 0);
        $this->db->where('tbl_members.email', $email);
        $query = $this->db->get();
    ...
    function createPasswordUser($email, $password)
    {
        $this->db->where('email', $email);
        $this->db->where('isDeleted', 0);
        $this->db->update('tbl_members', array('password'=>getHashedPassword($password)));
