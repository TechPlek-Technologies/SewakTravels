...
class Vehicle_model extends CI_Model
{
	...
	function getCabListings()
    {
        $this->db->select('id, vehicle_image, name, description, passengers, luggage, start_fee, fee_per_person, statusId');
        $this->db->from('tbl_cab_vehicle');
        $this->db->where('isDeleted', 0);
	...
	function addNewCab($data)
    {
        $this->db->trans_start();
        $this->db->insert('tbl_cab_vehicle', $data);        
        $insert_id = $this->db->insert_id();        
        $this->db->trans_complete();        
        return $insert_id;
    }
	function addCabVehicleDetails($data)
    {
		return $this->db->insert_batch('tbl_cab_vehicle_price', $data);
    }
	
	function checkCabVehicleExist($name, $id = null)
    {
        $this->db->select("id");
        $this->db->from("tbl_cab_vehicle");
		$this->db->where('isDeleted', 0);
    ...
    function getCabInfoById($id)
    {
        $this->db->select('t1.id, t1.name, t1.description, t1.passengers, t1.luggage, t1.start_fee, t1.fee_per_person, t1.vehicle_image, t1.statusId');
	...
	function vehicleRateList($id, $type = '')
    {	
		$this->db->select('id, cab_vehicle_id, trip_type, from, to, price, two_pay_price, additional_ride_time_fare, additional_km_fare, driver_allowance, night_charges');
		...
		$this->db->where('isDeleted', 0);
        $this->db->order_by('id ASC, trip_type ASC');
        $query = $this->db->get();
        $result = $query->result();        
        return $result;
    }
    ...
    {
        $this->db->where('id', $id);
        $this->db->update('tbl_cab_vehicle', $data);
        
        return $this->db->affected_rows();
    }
	
    function deleteCab($id, $data)
    {		
		$this->db->where('id', $id);
        $this->db->update('tbl_cab_vehicle', $data);
	...
	function deleteCabRateDetails($id, $dataArr)
    {
		
		//$this->db->delete('tbl_cab_vehicle_price', array('cab_vehicle_id' => $id));
		
		$this->db->where('cab_vehicle_id', $id);
        $this->db->update('tbl_cab_vehicle_price', $dataArr);
		
		return true;
    }


    function checkMinAmountExist()
    {
        $this->db->select("id");
    ...
    function addMinAmounts($data)
    {
        $this->db->trans_start();
        $this->db->insert('tbl_min_amount', $data);
    ...
    function editMinAmount($data, $id)
    {
        $this->db->where('id', $id);
        $this->db->update('tbl_min_amount', $data);

        //$this->db->affected_rows();
        return TRUE;
    }

    function getMinAmountInfo()
    {
        $this->db->select('id, price, statusId');
        $this->db->from('tbl_min_amount');
        $query = $this->db->get();
